	...
	def Verificacion(self):
		...
		if self.numero.isalpha()==True:
			...
			self.numero.find("VVVV")==-1 and self.numero.find("LLLL")==-1) :		
				...
				print("Roman number: ",self.numero," has a valid alphabetical format")
				y=True
				return y
			else:
				print("Roman number: ",self.numero," is not valid, please try again")	
				y=False
				return y
		else:
			print("Roman number: ",self.numero," is not valid, please try again")
	...
	def Desromanizar (self):
		...
		if comprobante2==True:
			while p<cantidad:
				...
				p=p+1
			print("Roman number: ",self.numero, " is equivalent to: ",str(decimal))
			y=True
			return y
		else:
			print("Roman number: {} has an invalid form, check if the number has the correct syntaxis")
			print("Remember that numbers like IL are not valid, the apropiate form is XLIX")
			y=False
			return y
...
while comprobante==False:
	numero_recibido=input("Insert the roman number you´d like to transform: ")
